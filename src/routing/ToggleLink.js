...
export class ToggleLink extends Component {
    ...
    render() {
        return <Route path={ this.props.to } exact={ this.props.exact } children={ routePorps => {
            ...
            const combinedClasses = `${baseClass} ${routePorps.match ? activeClass : inActiveClass}`;

            return <React.Fragment>
                { this.state.doRedirect && <Redirect to={ this.props.to } /> }
                <button className={ combinedClasses } onClick={ () => this.handleClick(routePorps.history) }>
                    { this.props.children }
                </button>
            </React.Fragment>
        }} />
